        ...
        <div className="w-full grid grid-cols-3 gap-[30px] mt-5">
            {studentAttendances?.map((card, index) => (<div key={index} className="border w-full h-[307px] rounded-[12px] p-[16px] shadow-[0px_4px_20px_rgba(0,0,0,0.25)] grid gap-[20px]">
                ...
                <div className="flex justify-between w-full items-center pl-2 pr-7">
                    {card?.students.map((img, index) => (<img key={index} className='w-[62px] h-[62px] rounded-full bg-[#000000] border-2 border-white 
                  -ml-50 first:ml-0 z-${index}' src={GetImageUrl(img.student.image) ?? undefined} />))}
                ...
                <div className="flex  w-full justify-between gap-0.5 ">
                    <p className={`text-[${COLORS.gray_light}] font-semibold flex gap-[7px] items-center`} style={{ ...FONTS.heading_08 }}> <TbCalendarTime className="w-[18px] h-[18px]" />{dayjs(card?.student_class?.start_date).format("DD-MMM-YYYY")}</p>
