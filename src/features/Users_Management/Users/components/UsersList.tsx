...
const UsersList: React.FC<props> = React.memo(
					...
					Users?.map((card: any, index: any) => (
						...
						>
							...
							<div className='flex gap-3 sm:gap-3 md:gap-4 items-start justify-between relative'>
								...
								<div className='flex items-start h-full flex-shrink-0'>
									<button
										...
										onClick={() =>
											setCardOptionsShow(
												cardOptionsShow === index ? null : index
											)
										}
									>
										<SlOptionsVertical
											className={`text-[${COLORS.primary}] text-lg sm:text-xl`}
										/>
									</button>
									{cardOptionsShow === index && (
										<div
											ref={dropdownRef}
											className='absolute right-0 top-8 sm:top-9 z-10'
										>
											<CardOptions uuid={card?.uuid} fetchUser={fetchUser} />
										</div>
									)}
								</div>
							</div>
							...
							<div className='flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 sm:gap-3 md:gap-4'>
								<p
									className={`text-[${COLORS.light_green}] text-xs sm:text-sm md:text-sm`}
								>
