...
export default function AllNotifications() {
	...
	const totalPages = notifications?.last_page || 1;
	...
	const readCount =
		notifications?.data?.filter(
			(n: any) => n.status?.toLowerCase() === 'delivered'
		).length || 0;
	const unreadCount =
		notifications?.data?.filter(
			(n: any) => n.status?.toLowerCase() !== 'delivered'
	...
	const stats = [
		...
		{
			title: 'Not Delivered Notifications',
			count: unreadCount,
			color: 'bg-[#E3418F33]',
			image: classImg,
		},
	...
	return (
		<div className='p-6 w-full pb-20 relative'>
			{/* Header */}
