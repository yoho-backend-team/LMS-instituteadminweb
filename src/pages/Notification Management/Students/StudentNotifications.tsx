...
const StudentNotifications = () => {
	...
	const reponse = useSelector(selectStudentNotification);
	const dispatch = useDispatch<any>();
	const coursedata = useSelector(selectAllCourse)?.data;
	const [students, setStudents] = useState<any>([]);
	const totalPages = reponse?.last_page;
	...
	}, [branchId, dispatch, instituteId, currentPage]);
	...
	return (
		<div className='p-6 min-h-screen'>
							...
							<Select
								...
								onValueChange={(value) => {
									if (selectedStudent.includes(value)) {
										setSelectedStudent(
											selectedStudent.filter((v) => v !== value)
										);
									} else {
			...
			<div>
				...
				{totalPages > 1 && (
					<div className='fixed bottom-0 w-full bg-white border-t shadow-md py-3 px-4 flex justify-center items-center gap-4 z-50'>
						<Button
							variant='outline'
