...
export const formatDateWithDay = (iosString: string): string => {
  const date = new Date(iosString);
...
export const formatDateandTime = (iosString: any): any => {
  const date = new Date(iosString);
  const options: Intl.DateTimeFormatOptions = {
    day: 'numeric',
    month: 'long',
...
export const formatTime12h = (timeString: string): string => {
  // If it's already a proper time range, just return as-is
  if (timeString.includes('-')) {
    return timeString.trim();
  }

  // If it's a plain 12h time like "12:00 PM"
