...
interface formtype {
	...
	setStatusFilter: (data: 'all' | 'present' | 'absent') => void;
	statusFilter: 'all' | 'present' | 'absent'; // ✅ Added to track active filter
}
...
const StaffAddBar: React.FC<formtype> = ({
	...
	const handleFilter = (type: 'all' | 'present' | 'absent') => {
		setStatusFilter(type);
		setfilterDiv(false);
	};

	// ✅ helper to apply active styles
	const filterButtonClass = (type: 'all' | 'present' | 'absent') =>
		`border-2 text-[16px] font-medium rounded-md py-1 px-2 transition ${
			statusFilter === type
				? 'bg-[#1BBFCA] border-[#1BBFCA] text-white'
				: 'border-[#716F6F] text-[#716F6F] hover:text-white hover:bg-[#1BBFCA] hover:border-[#1BBFCA]'
		}`;
	...
	return (
		<div className='flex flex-col w-full gap-4 p-2'>
			...
			<div className='flex flex-col sm:flex-col md:flex-row w-full justify-between items-center p-4 bg-white rounded-lg shadow-[0px_4px_24px_0px_#00000026]'>
				...
				<div
					className='mt-4 md:mt-0 flex justify-start md:justify-end w-[200px] md:w-[199px] bg-[#3ABE65] h-min p-2 text-white rounded-md items-center gap-1 cursor-pointer'
			...
			<div className='flex flex-row justify-between w-full relative'>
				{/* Calendar button */}
				<div
					className='p-4 bg-green-100 rounded-md cursor-pointer'
				...
				{filterDiv && (
					<div className='absolute top-full right-0 mt-2 z-10 flex flex-col rounded-lg p-4 gap-3 bg-white shadow-[0px_4px_24px_0px_#00000026] w-[150px]'>
						<button
						...
						<button
							...
							className={filterButtonClass('present')}
						>
							Present
						</button>
						<button
							onClick={() => handleFilter('absent')}
							className={filterButtonClass('absent')}
						>
							Absent
						</button>
					</div>
