...
const SalaryTable: React.FC<SalaryTableProps> = ({
	...
	const filteredData = AllSalary.filter((row: any) => {
		...
		const matchesBranch = branch === '' || row.branch_id === branch;
								...
								<div className='flex justify-between items-start mb-3'>
									<div>
										...
										<div className='flex items-center gap-2 mt-1'>
											<div className='w-10 h-10 rounded-full text-white bg-[#1BBFCA] flex items-center justify-center font-semibold'>
												{row?.staff?.username?.charAt(0)}
											</div>
											<div>
												<p className='font-semibold text-gray-800'>
													{row?.staff?.username || 'N/A'}
												</p>
						...
						{loading ? (
							...
							<tbody>
								{filteredData.map((row: any) => (
									<tr
										key={row.id}
										className='border-t text-sm font-semibold hover:bg-gray-50'
									>
											...
											<div className='flex items-center gap-3'>
												...
												<div className='flex flex-col'>
													...
													<span className='text-sm text-gray-500'>
														{row.staff?.staffId ? row.staff?.staffId : '-'}
													</span>
												</div>
											</div>
										</td>
						...
						<div className='p-4'>
							<div className='w-full flex flex-col justify-center items-center gap-4 mb-6'>
								<div className='w-24 h-24 lg:w-32 lg:h-32 border rounded-full overflow-hidden'>
									<img
										src={
											selectedCard.staff?.profileImage ||
							...
							<form className='grid grid-cols-1 md:grid-cols-2 gap-4'>
								...
								<div className='flex flex-col'>
									...
									<input
										value={
											selectedCard?.email ? selectedCard.staff?.email : 'Nil'
										}
										readOnly
										className='h-10 border rounded p-2 bg-gray-100 text-sm'
									/>
