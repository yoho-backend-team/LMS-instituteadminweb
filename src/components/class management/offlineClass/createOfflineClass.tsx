...
export const CreateOfflineClassModal = React.memo(
	({ isOpen, setIsOpen, fetchAllOfflineClasses }: CreateBatchModalProps) => {
		...
		useEffect(() => {
			(async () => {
				...
				try {
					...
					const staffRes = await getStaffService({
									...
									>
										...
										<SelectContent className='bg-white'>
											{availableInstructors?.map((instructor) => (
												<SelectItem
													className={`cursor-pointer hover:bg-[${COLORS.primary}]`}
													key={instructor._id}
