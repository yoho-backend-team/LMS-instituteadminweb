...
const RefundAdd: React.FC<RefundAddProps> = ({
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		if (editData?.uuid) {
			...
			try {
				const res = await updateRefund(updatePayload);
				if (res) {
					toast.success('Refund updated successfully!');
				}
			} catch (error) {
				toast.error('Failed to update refund. Invalid refund');
			}
		} else {
			try {
				const res = await dispatch(CreateRefundThunk(apiPayload));
				if (res) {
					refundCounter++;
					toast.success('Refund saved successfully!');
				}
			} catch (error) {
				toast.error('Failed to create refund. Invalid refund');
			}
		}
			...
			>
				<div className='space-y-4'>
					...
					<div className='flex flex-col p-1'>
						...
						{branchId && selectedCourse && batches?.length === 0 && (
							<p className='text-red-500 text-sm mt-1'>
