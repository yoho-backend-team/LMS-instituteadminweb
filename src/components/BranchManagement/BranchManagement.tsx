...
export function LocationCardsGrid() {
  ...
  useEffect(() => {
    dispatch(fetchBranches({ instituteId: "YOUR_INSTITUTE_ID" }));
  }, [dispatch]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingBranch) {
        ...
        await dispatch(createBranch({ instituteId: "YOUR_INSTITUTE_ID", data: branchData })).unwrap();
      }
  ...
  const handleDeleteBranch = async (branchId: string) => {
    try {
      await dispatch(deleteBranchAction(branchId)).unwrap();
      setShowSuccessPopup(true);
    } catch (error) {
