...
import {
	GetViewCard,
	UpdateGroup,
} from '../../../features/Users_Management/Group/reducers/service';
...
function Edit() {
	...
	const handleSave = async () => {
		...
		const noPermissions = Object.entries(permissions)
			// ignore non-permission fields like 'identity', 'id', '_id', etc.
			.filter(([key]) => !['identity', 'id', '_id', 'name'].includes(key))
			// check each module’s permissions
			.every(([, module]) =>
				Object.values(module).every((perm: any) => perm === false)
			);
		...
		try {
			...
			const res = await UpdateGroup(payload); // Submit to backend
			if (res) {
