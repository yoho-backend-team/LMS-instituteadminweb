...
const FaqCategory: React.FC = () => {
	...
	const [isDeleteSuccessModalOpen, setIsDeleteSuccessModalOpen] =
		useState(false); // New state for delete success
	...
	const handleSubmit = async () => {
		...
		if (isEditing && editCategoryId !== null) {
			...
			try {
				...
				toast.error('Failed to add FAQ category. Please try again.');
			}
		}
				...
				<div className='fixed inset-0 bg-[#7D7D7D] bg-opacity-40 flex items-center justify-end z-50 px-4 '>
					<div className='bg-white rounded-lg p-6 w-full max-w-md shadow relative  h-[90%]'>
						...
						<input
							...
							className={`border w-full px-4 py-2 rounded mb-1 ${
								titleError ? 'border-red-500' : 'border-gray-400'
							}`}
						/>
						{titleError && (
							<p className='text-red-500 text-xs mb-4'>{titleError}</p>
						)}
						...
						/>
						{descriptionError && (
							<p className='text-red-500 text-xs mb-4'>{descriptionError}</p>
						)}
						<div className='flex justify-end gap-4'>
							<button
								className='bg-gray-400 text-white px-6 py-2 rounded mt-3'
								onClick={() => {
									setIsFormOpen(false);
									setTitleError('');
									setDescriptionError('');
							...
							<button
								...
								className='bg-[#1BBFCA] text-white px-6 py-2 rounded mt-3'
							>
								Submit
							</button>
						</div>
