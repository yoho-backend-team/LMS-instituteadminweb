	...
	useEffect(() => {
		(async () => {
			try {
				...
				const responses = await getStaffService({
					uuid: formik?.values?.branch,
				});
				if (responses) {
	...
	return (
		<div className='fixed inset-0 z-50 overflow-y-auto'>
			<div className='fixed inset-0 z-50 flex items-center justify-center overflow-y-auto'>
				...
				{/* Modal container */}
				<div className='relative bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:max-w-3xl w-full mx-4'>
						...
						<form onSubmit={formik.handleSubmit} noValidate>
							<div className='space-y-4 max-h-[calc(100vh-200px)] overflow-y-auto mt-2 p-2'>
								...
								<div>
									...
									{formik.values.branch && allCourses?.length === 0 && (
								...
								<div>
									...
									{formik.values.course && allBatches?.length === 0 && (
										<p className='mt-1 text-xs text-red-500'>
								...
								<div>
									...
									{!formik.values.classDate && (
										<p className='mt-1 text-xs text-gray-500'>
											Please select above all fields first to enable instructor
											selection
										</p>
									)}
								</div>
