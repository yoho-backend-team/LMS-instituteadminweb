			...
			<div className='flex flex-col gap-4 w-full'>
				{loading ? (
					...
					batches.map((batch: any, index: number) => {
						...
						const statuses = batch?.last_message?.status || [];
						const unreadCount = Array.isArray(statuses)
							? statuses.filter((s: any) => s.read === false).length
							: 0;
						const allUnread =
							userIds?._id !== batch?.last_message?.sender &&
							unreadCount === statuses.length &&
							unreadCount > 0;
						...
						return (
							<div
								...
								className={`h-[80px] w-[270px] sm:w-[220px] lg:w-[270px] bg-white rounded-xl shadow-md p-3 flex items-center justify-between cursor-pointer transition ${
								...
								<div className='flex items-center'>
									...
									<div>
										...
										<p className='text-[#7D7D7D] text-xs'>
											{batch?.last_message?.message ?? 'no message'}
										</p>
									</div>
								</div>
